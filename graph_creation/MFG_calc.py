...
def MFG_calc(S, r, v):
    ...
    # Added to Handle possible numerical issues in J
    if np.any(np.isnan(J)) or np.any(np.isinf(J)):
        print("Warning: J matrix contains NaNs or Infs. Replacing them.")
        J = np.nan_to_num(J)

    # Regularization to ensure matrix inversion stability
    regularization = 1e-6
    J_regularized = J + regularization * np.eye(J.shape[0])

    # Calculate pseudoinverse
    J_pinv = np.linalg.pinv(J_regularized)
